...
var preimages = make(map[libcommon.Hash][]byte)
var root = "/tmp/cannon"
...
func Preimage(hash libcommon.Hash) []byte {
...
func Preimages() map[libcommon.Hash][]byte {
	return preimages
}
...
func (kw PreimageKeyValueWriter) Put(key []byte, value []byte) error {
	...
	if hash != libcommon.BytesToHash(key) {
