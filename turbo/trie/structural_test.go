...
func TestBranchesOnly(t *testing.T) {
	...
	for _, key := range keys {
		...
		succ.Write(key.k)
		if curr.Len() > 0 {
			groups, hasTree, hasHash, err = GenStructStep(func(_ []byte) bool { return false }, curr.Bytes(), succ.Bytes(), hb, hc /* hashCollector */, &GenStructStepHashData{common.Hash{}, key.hasTree}, groups, hasTree, hasHash, false, nil, false)
			if err != nil {
				t.Errorf("Could not execute step of structGen algorithm: %v", err)
			}
		}
	...
	succ.Reset()
	// Produce the key which is specially modified version of `curr` (only different in the last nibble)
	if _, _, _, err = GenStructStep(func(_ []byte) bool { return false }, curr.Bytes(), []byte{}, hb, hc /* hashCollector */, &GenStructStepHashData{common.Hash{}, false}, groups, hasTree, hasHash, false, nil, false); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
	require.Equal(t, 7, i)
}
...
func Test2(t *testing.T) {
		...
		if curr.Len() > 0 {
			...
			groups, hasTree, hasHash, err = GenStructStep(func(_ []byte) bool { return false },
				...
				func(keyHex []byte, hasState, hasTree, hasHash uint16, hashes, rootHash []byte) error {
					return nil
				}, /* hashCollector */ &GenStructStepHashData{Hash: common.BytesToHash(key.v)}, groups, hasTree, hasHash, false, nil, false)
			if err != nil {
				t.Errorf("Could not execute step of structGen algorithm: %v", err)
			}
		}
	...
	if _, _, _, err = GenStructStep(func(_ []byte) bool { return false }, curr.Bytes(), succ.Bytes(), hb, func(keyHex []byte, hasState, hasTree, hasHash uint16, hashes, rootHash []byte) error {
		return nil
	}, /* hashCollector */ &GenStructStepHashData{Hash: common.BytesToHash(keys[len(keys)-1].v)}, groups, hasTree, hasHash, false, nil, false); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
}
